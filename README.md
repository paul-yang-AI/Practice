...
**成本紀律與 AI 分工**：上表 **Train KPI** 刻意走 `--tier0-only`（`use_arbiter=False`）— Tier0（正則 + BS4）在 3 份 train 上 **100% required、$0/filing**，證明核心路徑可回歸、非「純 prompt 工程」。**Agent** 以 LLM 為主引擎（規劃 / 抽取 / 驗證）。**SEC 的 LLM** 為 **opt-in 手術刀**：邊界仲裁（Tier2）、結構分類、chunk fallback — Tier2 經 SEC 頁 checkbox 或 `--with-arbiter`；Tier1 fallback 經 CLI `--with-llm`（UI 預設關閉 fallback）；held-out 上可見增量（如 AAPL 2010：2/4→3/4 required）。Citi train **3/3** 由 Tier0 section_name / TOC scrub 達成，非預設 arbiter。全域預算 $20、每代理 $0.50、每財報 $0.30；超限 `BudgetExceeded`（見 `scripts/demo_circuit_breaker.py`）。

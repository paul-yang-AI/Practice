...
```mermaid
flowchart TB
  subgraph UI["Streamlit 多頁"]
    SEC_UI["SEC 10-K"]
    AG_UI["Browser Agent"]
    EV_UI["Eval 儀表板"]
  end

  subgraph Harness["shared_harness"]
    Router["llm_router"]
    Cost["cost_tracker"]
    Edgar["edgar_client"]
    EvalR["eval_runner"]
  ...
  subgraph SEC["Task 2 — SEC 10-K"]
    T0["Tier0 · BS4/regex · $0"]
    T1["Tier1 · classify / fallback · opt-in"]
    T2A["Tier2 · boundary arbiter · opt-in"]
    T0 --> T1 --> T2A
  end

  subgraph AG["Task 1 — Browser Agent"]
    Loop["Plan → Act → Observe → Verify"]
    PW["Playwright"]
    Loop --> PW
  end
...
> **用語說明：** 此處 **Train** = eval 切分（基準集 / KPI regression），**不是** fine-tune 或訓練模型權重；LLM 為固定 API（Gemini），Tier0 為規則管線。
...
│   ├── agent/
│   │   ├── loop.py         #   主狀態機（規劃→執行→觀察→驗證→Recovery）
...
│   └── eval/tasks.yaml     #   10 個代理評估任務（5 train + 5 held-out；含 2 smoke_only）
...
│       ├── gold/           #   train 邊界 regression 快照（Tier0 baseline）
...
```
導覽(Navigate) → LLM 規劃(Plan) → 執行(Act) → 觀察(Observe) → 驗證(Verify) → 恢復(Recovery, 失敗時)
```
...
- **邊界仲裁（Arbiter，Tier2）**：僅當某段「低信心」且開啟仲裁時，用 Tier2 模型在局部視窗內**只調整邊界偏移**（不改寫原文）。預設 train/held-out KPI 走 Tier0-only（`use_arbiter=False`）。**SEC 頁 checkbox** 或 CLI `--with-arbiter` 會傳 `run_id` 以觸發仲裁；train 三檔 Tier0 上 `low_confidence_count=0`，故**無已提交的 filing 級 KPI 增量**— arbiter 用於 UI 上爭議邊界除錯，非撐起 3/3 train。
...
- **K-1 內部頁碼引用（`K-xx`）**：像 BRK.B 這類 filing 在主文件 Part 中只列章節標題 + `K-24` / `K-33` 指標，完整 Risk Factors / MD&A 在 K-1 附錄。`content_quality.is_k1_exhibit_reference_index()` 將短段 `K-xx` 標為 **cross_ref**（UI 顯示「K-1 索引」），而非誤標 ✅ 真實內文。
...
| 公司 | Accession | 特點 / 表現 |
|---|---|---|
| **MSFT** | `0000950170-24-087843` | 6.8 MB iXBRL、標準 TOC；gold **22/22** 邊界；`tier0_extracted_count` **21**；required **4/4**；$0.00 |
...
| **BRK.B** | `0000950170-25-025210` | Heldout，K-1 式 TOC；1A/7 可能為 `K-xx` 交叉引用（非完整正文）；required 4/4 仍可過（cross-ref 誠實標記） |

**反過度擬合措施**：管線程式碼裡**沒有任何按 ticker / accession 的特判分支**；覆蓋率以「完整正規化 body」為分母計算；`eval/gold/` 為 **Regression baseline**（Tier0 快照 + 邊界 ±5 字元比對），輔以 `docs/eval_spot_checks.md` 抽樣人工 spot-check。
...
- **SEC**：讀 `task2_sec/eval/manifest.json` → 按 split 過濾 → 抓取 + 抽取 → 對照 gold 邊界與契約指標評分。
  ...
  - **Gold 邊界（Regression）**：`gold_items = ["1","1A","1B","1C","7","8","10","14"]` — 第二層防退化測試（start/end ±5），與 required KPI 分開；非公開 ground truth。
...
### Held-out（8 檔；manifest 共 11 檔 = 3 train + 8 held-out，依 cache 執行）
...
**方法論限制**：缺乏公開 ground truth 時，**主正確性防線**為確定性契約（span / token / header）+ `required_items` 的 **content-quality**（TOC stub 不算通過）；`eval/gold/` 為 **Regression baseline**（改 segment 邏輯時防邊界漂移），並以 spot-check 抽樣補足。
...
| **擴充 manifest** | **8 檔** held-out（BRK + 7 optional 變異軸；manifest 共 11 檔）；`cache_heldout_filings.py` |
...
| 靜默失敗（silent failure） | N/A（規則管線，無 agent 式 done 宣告） | **0** |
| 每單位延遲 P50 / P95 | 單份秒級（本地，無 LLM） | **~5.9s / ~8.1s**（見 `eval_summary.json`） |
| 每單位成本 P50 | **$0.00 / filing**（Tier0 全覆蓋，零 LLM） | **~$0.0031 / task** |
| LLM 呼叫總數 | 0（eval 路徑） | 8 |
| Token 比率 P50 | 0.98–0.99（保證原文、非摘要） | — |

**成本紀律與 AI 分工**：上表 **Train KPI** 刻意走 `--tier0-only`（`use_arbiter=False`）— Tier0（正則 + BS4）在 3 份 train 上 **100% required、$0/filing**，證明核心路徑可回歸、非「純 prompt 工程」。**Agent** 以 LLM 為主引擎（規劃 / 抽取 / 驗證）。**SEC 的 LLM** 為 **opt-in 手術刀**：**Tier1** chunk fallback（CLI `--with-llm` / `run_heldout_baseline.py --with-llm`）；**Tier2** 邊界仲裁（SEC 頁 checkbox 或 `--with-arbiter`，僅 `low_confidence` 段）。Held-out `with_llm` 快照：整體仍 **6/8 ok**；AAPL 2010 在當前 baseline 仍 **2/4** required（誠實 gap）。Citi train **3/3** 由 Tier0 section_name / TOC scrub 達成。提交用 train CSV 請以 **`run_agent_eval.py` 最後寫入**（3 SEC + 5 agent 列）。全域預算 $20、每代理 $0.50、每財報 $0.30；超限 `BudgetExceeded`（見 `scripts/demo_circuit_breaker.py`）。
...
此外有 **baseline 對比**（regex-only vs 樸素 LLM vs 本混合管線：必需項召回 66.7% / 50% / **100%（per-filing required contract）**，成本 $0 / $3.17 / **$0**）佐證設計取捨；代理端則以 **L0 每步驗證 + 可選 Blind Critic** 防止「成功卻沒結果」的靜默失敗。`task2_sec/eval/gold/` 為 **Regression baseline**（Tier0 快照；`scripts/regenerate_gold.py`）；**品質主保證**仍是上述確定性契約 + required content-quality，gold 用於防 refactor 退化。
...
以下**選填**變數若未在 `.env` 或 Zeabur 設定，程式會使用**內建預設值**（與下表相同），不影響正常啟動；僅在要改行為時才需覆寫。

| 變數 | 必填 | 未設定時的預設 | 說明 |
|---|---|---|---|
| `GEMINI_API_KEY` | 是 | — | Tier1/Tier2 主用模型 |
| `SEC_USER_AGENT` | 是 | — | SEC 要求的 UA，格式：`"公司名 聯絡人 email@domain.com"` |
| `RUN_BUDGET_USD` | 否 | **`20`** | 全域 LLM 花費熔斷上限（USD） |
| `ENABLE_BLIND_CRITIC` | 否 | **`false`** | 設 `true` 啟用代理終局盲審（多一次 LLM、更嚴） |
| `ENABLE_SEC_LLM_CLASSIFY` | 否 | **`false`** | 設 `true` 時，Tier1 對 UNKNOWN 段做結構分類 |
| `OPENROUTER_API_KEY` | 否 | — | fallback 模型用；無 key 時 fallback 自動跳過 |
| `LLM_FALLBACK_ENABLED` | 否 | **`true`** | 設 `false` 關閉 OpenRouter fallback（純 Gemini） |
| `SQLITE_JOURNAL_MODE` | 否 | **`WAL`** | 單容器 Zeabur 用 WAL 即可；NFS/多實例可改 `TRUNCATE` |
| `LLM_TIER{1,2}_{PRIMARY,FALLBACK}` | 否 | 見 `.env.example` | 覆寫預設模型 ID |
...
| 12+ 個代理任務 | **10 個**（5 train + 5 held-out；含 2 smoke_only） | 證明「恢復能力」，而非堆成功率 |
...
7. **語意抽取 / RAG 層（可選）**：在現有 Item span（`body[start:end]`）之上，對長段落做 chunk + LLM，支援 MD&A 重點、風險主題等**非 verbatim** 用例；輸出需帶 citation 回 span offset，**不取代** Tier0 合規切分。不採用「整份 filing 一次 LLM 生成各 Item 正文」作預設路徑（`run_baselines.py`：naive LLM 長 filing required recall 較低、token ratio ~0.4、成本高於 Tier0 $0）。
8. **Exhibit 內聯追蹤**：對 `K-xx` / incorporation 交叉引用，可選擇抓取並切分 K-1 附錄 span（需額外 EDGAR 文件解析；目前僅在主文件誠實標 cross-ref）。

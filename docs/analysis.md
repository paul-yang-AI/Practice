...
以下數字除非標註 *estimated（估算）*，皆來自 **`reports/eval_train.csv`**、`reports/eval_summary.json`、`reports/heldout_baseline.json`（SEC held-out 基線以本地執行時間為準）。

---

## Executive Summary（執行摘要）

1. **SEC train：成本與 required 召回** — 3/3 train filings 通過 per-filing **required** contract（MSFT **4/4**、INTC/Citi **3/3**）；Eval KPI 路徑 **Tier0-only**，**$0.00/filing**（span / token / header 契約驗證，非 E2E LLM 生成正文）。
2. **Agent train：零 silent failure** — **5/5** success、**silent_failure=0**（L0 每步 verify + 終局 outcome 檢查）；Blind Critic 為 **opt-in**（Zeabur 預設 off）。Held-out **3/5** ok（forms / python_docs / quotes；DDG / SEC EDGAR 為誠實 gap）。
3. **防禦性泛化** — 管線**無 ticker 特判**；8 檔 SEC held-out **6/8** `failure_category=ok`（AAPL 2010、KSCP 10-K/A 為預期 Tier0 gap）。Citi / JPM mega-TOC、BRK K-xx 靠通用 fallback + `content_quality` 誠實標記，而非為單一公司寫死規則。
...
- **Required items recall（必需項召回）**：MSFT **4/4**；INTC **3/3**；Citi **3/3**（per-filing `required_items` override）。
...
- **Silent failures**：**0**
- **P50 latency**：**~5.9s**；**P95**：**~8.1s**（見 `reports/eval_summary.json`）
- **P50 cost**：**~$0.0031/task**
- **Recovery steps（總計）**：**1**
...
† **INTC Regex 4/4**：baseline 腳本用 manifest 預設 `required_items`（含 Item 1）；Hybrid **3/3** 為 filing override（僅 1A/7/8）。
...
| **Agent Hybrid（本 repo）** | **~$0.0031** | Train 100%（5/5）；silent_failure=0 | 高（L0 + 可選 Critic） |
...
**擴充 held-out manifest**（**8 檔** held-out；manifest **共 11 檔** = 3 train + 8 held-out；`cache_optional` 項需 cache 才跑 — 見 `scripts/cache_heldout_filings.py`）。手動 spot-check：`docs/eval_spot_checks.md`。
...
| K-1-style TOC | BRK.B | 4/4 | ok | 1A/7 可能為 `K-xx` cross-ref（誠實標記） |
...
**Held-out + Tier1 LLM fallback**（`run_heldout_baseline.py --with-llm`，見 `reports/heldout_baseline.json` `with_llm` 區塊）：整體仍 **6/8 ok**；AAPL 2010 required **2/4**（與 Tier0 相同於當前快照，仍 `missing_item_header`）；KSCP 未改善。展示 Tier1 為可選增量路徑，非 end-to-end 替代 Tier0。

**Tier2 arbiter**：僅在 `low_confidence` + `use_arbiter` + `run_id` 時觸發；train Tier0 上幾乎無 low_confidence 段，**無 filing 級 KPI 對照**；SEC UI checkbox 供單段邊界除錯。
...
## LLM vs Tier0 策略（高風險領域抽取）

**高風險領域 RAG 哲學**（合規、醫療、量化等）適用於 SEC 10-K：**LLM 僅產出結構化決策與邊界座標**（offset、分類、仲裁）；**最終正文一律由程式自 HTML 切出**（`body[start:end]`），以 span integrity 保證可稽核、消除幻覺。不採「整份 filing chunk + LLM 複述各 Item」作預設路徑（見下方 Baseline 對照；Naive LLM 為 *estimated*）。
